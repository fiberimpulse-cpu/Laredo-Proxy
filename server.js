...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fbtn { padding: 7px 16px; border: none; border-radius: 6px; cursor: pointer; font-size: 0.68rem; font-family: monospace; letter-spacing: 0.1em; font-weight: 700; background: #1e293b; color: #94a3b8; }
  ...
  .card { background: #0f172a; border-radius: 10px; padding: 18px; position: relative; border-top: 3px solid #10b981; }
...
async function scan() {
  ...
  try {
    ...
    for(let page=1;page<=2;page++) {
      ...
      raw.forEach(p=>{
        ...
        if(price>0) results.push({
          ...
          city: p.addressCity||p.city||'Laredo',
          zip: p.addressZipcode||p.zipcode||'',
          price,
          beds: Number(p.bedrooms||0),
          baths: Number(p.bathrooms||0),
          sqft: Number(p.livingArea||0),
          type: p.homeType||'',
          dom: Number(p.daysOnZillow||p.daysOnMarket||0),
          cuts: Number(p.priceReductionCount||0),
    ...
    if(results.length===0) {
      document.getElementById('error').textContent='⚠ No listings found. Try again.';
